...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, episode_ids, dataset_dir, camera_names, norm_stats, arm_delay_time,
                 use_depth_image, use_audio, use_robot_base):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            # action started from the second timestep, last action duplicated to avoid slicing problem
            ...
            audio_r = None
            if self.use_audio:
                    audio_l = root['/observations/audio/audio_l'][start_ts]
                    audio_r = root['/observations/audio/audio_r'][start_ts]
        ...
        if self.use_audio:
            audio_l = norm_audio(audio_l, self.norm_stats["audio_max"][0], self.norm_stats["audio_min"][0])
            audio_r = norm_audio(audio_r, self.norm_stats["audio_max"][1], self.norm_stats["audio_min"][1])
            audio_l = torch.from_numpy(audio_l).float()
            audio_r = torch.from_numpy(audio_r).float()
            audio_data = torch.stack([audio_l, audio_r], dim=0)
        ...
        return image_data, image_depth_data, qpos_data, action_data, action_is_pad, audio_data

def norm_audio(audio_data, max_value, min_value):
    audio_data = np.array(audio_data)
    normalized_audio = (audio_data - min_value) / (max_value - min_value)
    normalized_audio = normalized_audio * 2 - 1 
    return normalized_audio

# TODO: add normed stats for gelsight and audio
def get_norm_stats(dataset_dir, num_episodes, use_audio, use_robot_base):
    all_qpos_data = []
    all_action_data = []
    all_audio_data_l = []
    all_audio_data_r = []
    ...
    for episode_idx in range(num_episodes):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            if use_audio:
                audio_data_l = root['/observations/audio/audio_l'][:]
                audio_data_r = root['/observations/audio/audio_r'][:] 
                all_audio_data_l.append(torch.from_numpy(audio_data_l))
                all_audio_data_r.append(torch.from_numpy(audio_data_r))
    ...
    if use_audio:
        all_audio_data_l = torch.cat(all_audio_data_l, dim=0).flatten()
        all_audio_data_r = torch.cat(all_audio_data_r, dim=0).flatten()
        audio_l_min = all_audio_data_l.min().item()
        audio_l_max = all_audio_data_l.max().item()
        audio_r_min = all_audio_data_r.min().item()
        audio_r_max = all_audio_data_r.max().item()
        audio_min = (audio_l_min, audio_r_min)
        audio_max = (audio_l_max, audio_r_max)
    else:
        audio_min, audio_max = None, None
    ...
    stats = {
        ...
        "example_qpos": all_qpos_data[0].numpy(),  # Assuming you want the first episode's qpos as example
        "audio_min": audio_min,
        "audio_max": audio_max
    }
...
def load_data(dataset_dir, num_episodes, arm_delay_time, use_depth_image, use_audio,
    ...
    norm_stats = get_norm_stats(dataset_dir, num_episodes, use_audio, use_robot_base) #TODO
