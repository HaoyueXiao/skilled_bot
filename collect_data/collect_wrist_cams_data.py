...
def save_data(args, timesteps, actions, dataset_path):
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        ...
        if args.use_depth_image:
            ...
            for cam_name in args.camera_names:
                _ = image_depth.create_dataset(cam_name, (data_size, 480, 640), dtype='uint8',
                                             chunks=(1, 480, 640), )
...
class RosOperator:
    ...
    def get_frame(self):
        ...
        if self.args.use_depth_image:
            frame_time = min([self.img_left_deque[-1].header.stamp.to_sec(), self.img_right_deque[-1].header.stamp.to_sec(),
                              self.img_left_depth_deque[-1].header.stamp.to_sec(), self.img_right_depth_deque[-1].header.stamp.to_sec()])
        else:
            frame_time = min([self.img_left_deque[-1].header.stamp.to_sec(), self.img_right_deque[-1].header.stamp.to_sec()])
        ...
        return (img_left, img_right, img_left_depth, img_right_depth,
    ...
    def process(self):
        ...
        print_flag = True
        global exit_flag
        while (self.args.use_keyboard_end or count < self.args.max_timesteps + 1) and not rospy.is_shutdown():
            if exit_flag:
                break
            # 2 收集数据
            ...
            print_flag = True
            count += 1
            (img_left, img_right, img_left_depth, img_right_depth,
            ...
            if self.args.use_depth_image:
                ...
                image_dict_depth[self.args.camera_names[0]] = img_left_depth
                image_dict_depth[self.args.camera_names[1]] = img_right_depth
...
def get_arguments():
    ...
    parser.add_argument('--camera_names', action='store', type=str, help='camera_names',
                        default=['cam_left_wrist', 'cam_right_wrist'], required=False)
